...
<head>
<title>JWS P10 Setting Page</title>
<style> body { width: 97% ; text-align: center; } input, select, button, textarea { max-width: 100% ; margin: 5px; padding: 5px; border-radius: 7px; } meter, progress, output { max-width: 100% ; } h1 { color: grey; } .note { color: #555; font-size:1em;} .info { background-color: #eee; border-radius: 3px; padding-top: 10px; padding-bottom: 10px; margin: 5px;} .tombol { background-color: #ccc; margin: 20px; padding: 10px; border-radius: 10px; text-decoration: none;}</style>
</head>
...
<h2>Setting Parameter JWS P10</h2>
<br>
<h4><a href="https://wa.me/6285862929263">Habib Muhammad</a> X <a href="http://elektronmart.com">ElektronMart.Com</a></h4>
...
<br><hr/><br>
<h2>Atur Lama Waktu Iqomah & Adzan (Menit)</h2>
...
<table width='100%'>
  ...
  <tr>
    ...
    <td>
      <label for="durasiadzan">Lama Sholat</label><br>
...
<br><br>
<h2>Koordonat Lokasi & nilai Kalibrasi</h2>
...
<h2>Atur Teks Informasi JWS</h2>
<form>
  <div>
    <label for="namamasjid">Nama Masjid</label><br>
    <textarea rows="3" id="namamasjid" placeholder="Mushola Nurul Huda"/></textarea>
...
<h2>Atur Nama WiFi Untuk JWS</h2>
<br><br><br>
<form>
<table width='100%'>
  <tr>
    <td>
      <label for="ssidData">SSID</label><br>
      <textarea id="ssidData" placeholder="JWS"/></textarea>
    </td>
    <td>
      <label for="passData">PASSWORD</label><br>
      <textarea id="passData" placeholder="12345678"/></textarea>
    </td>    
  </tr>
</table>
<br>  
...
<hr/>
<br><br>
<a class ='tombol' href="https://wa.me/6285862929263">Klik disini untuk Kontak Pembuat JWS</a><br><br><br><br><br>
<a class ='tombol' href="https://github.com/HabibMuhammad05">Habib Muhammad</a><span> X </span>
<a class ='tombol' href="https://github.com/busel7">Bonny Useful</a><br>
...
function handleServerResponseDataJWS(){
 ...
 if(xmlHttp.readyState==4 && xmlHttp.status==200){
   ...
   xmldoc = xmlResponse.getElementsByTagName('ssidData');
   message = xmldoc[0].firstChild.nodeValue;
   document.getElementById('ssidData').value=message;
   
   xmldoc = xmlResponse.getElementsByTagName('passData');
   message = xmldoc[0].firstChild.nodeValue;
   document.getElementById('passData').value=message;
 }
...
function setJws() {
  ...
  var ssidData = document.getElementById("ssidData").value;
  var passData = document.getElementById("passData").value;
  
  var datajws = {iqmhs:iqmhs, iqmhd:iqmhd, iqmha:iqmha, iqmhm:iqmhm, iqmhi:iqmhi, durasiadzan:durasiadzan, ihti:ihti, latitude:latitude, longitude:longitude, zonawaktu:zonawaktu, namamasjid:namamasjid, ssidData:ssidData, passData:passData};
