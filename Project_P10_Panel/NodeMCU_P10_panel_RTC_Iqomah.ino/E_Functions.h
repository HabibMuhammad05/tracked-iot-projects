void brightSet(){
  if ( (_hour24 >= 0 && _hour24 <= 7) || (_hour24 >=17 && _hour24 <= 23)){ Disp.setBrightness(5); }
...
void scrolling_date() {  
  ...
  static uint32_t pM;
  uint32_t cM = millis();
  teks = String(String(nameoftheday[_dtw-1]) + ", " +String(_day) + "-" + String(month_name[_month-1]) + "-" + String(_year) + " " + PesanTeks + " ");
...
void Iqomah() {
  ...
  Disp.setFont(SystemFont5x7);   
  Disp.drawText(1, 7, (menitiqmh < 10 ? "0" : "") + String(menitiqmh));
  Disp.drawText(20, 7, (detikiqmh < 10 ? "0" : "") + String(detikiqmh));
  Disp.drawText(14, 7, (detikiqmh ? ":" : " "));
